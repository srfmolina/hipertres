...
/// Observer: a left click on a cell runs `click_cell` on it.
///
...
/// entity. The observer sees clicks on *every* entity; `click_cell` ignores
/// the ones that aren't cells.
fn toggle_on_click(click: On<Pointer<Click>>, mut commands: Commands) {
    if click.button == PointerButton::Primary {
        // Run the `click_cell` system with the clicked entity as its input.
        commands.run_system_cached_with(click_cell, click.entity);
    }
}

/// Does what a click on `cell` does: toggles it.
///
/// It's a system with an *input* (`In<Entity>`), so it can also be run on
/// demand, without a real mouse click: `world.run_system_cached_with(click_cell, cell)`.
/// The tests use that to simulate clicks.
pub fn click_cell(
    In(entity): In<Entity>,
    mut cells: Query<(&mut Cell, Option<&ChildOf>)>,
    pressed_colors: Query<&PressedColor>,
) {
    // `get_mut` returns `Err` when the entity has no `Cell`: not a cell, ignore it.
    let Ok((mut cell, child_of)) = cells.get_mut(entity) else {
        return;
    };
    cell.pressed = match cell.pressed {
        Some(_) => None,
        // `ChildOf` exists only if the cell has a parent. Ask the parent for
        // its `PressedColor`, and fall back to the default.
        None => Some(
            child_of
